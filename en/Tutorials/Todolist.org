* The official Ramaze todolist tutorial
...
Take a look at the `src/element/page.rb` If the file and/or directory does not exist you will need to create it. In which case you will also need to reference the it as so, `require 'src/element/page'`, which is often placed in the `start.rb` file.
